...
def main():
    parser = argparse.ArgumentParser(description="Run Random Forest model")
    ...
    parser.add_argument("--result_file", default="results_random_forest.txt", help="Path to results txt file")
    ...
    for mode in ["all", "selected"]:
        print(f"\nRunning random_forest for mode: {mode}")
        log(f"\n>>> STARTING random_forest __{mode}", args.result_file)
        ...
        evaluate(model, Xtr, Xte, y_train, y_test, "random_forest", mode, args.result_file)

    print("\nRandom Forest evaluations done.")
