...
def plot_texture_analysis(df_sample, load_full_image, load_center_crop):
    for track_name, loader in [("Full96", load_full_image), ("Crop32", load_center_crop)]:
        records = []
        for _, row in tqdm(df_sample.iterrows(), total=len(df_sample), desc=f"Texture {track_name}", leave=False):
        ...
        for _, row in tqdm(df_sample.iterrows(), total=len(df_sample), desc=f"LBP {track_name}", leave=False):
            img = loader(row["filepath"])
        ...
        for cls, row_idx in [(0, 0), (1, 1)]:
            subset = df_sample[df_sample["label"] == cls].sample(4, random_state=SEED)
