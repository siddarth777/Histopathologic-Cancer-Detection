...
def main():
    parser = argparse.ArgumentParser(description="Run Logistic Regression model")
    ...
    parser.add_argument("--result_file", default="results_logistic_regression.txt", help="Path to results txt file")
    ...
    for mode in ["all", "selected"]:
        print(f"\nRunning logistic_regression for mode: {mode}")
        log(f"\n>>> STARTING logistic_regression __{mode}", args.result_file)
        ...
        model = LogisticRegression(**study.best_params, max_iter=1000, solver="lbfgs")
        evaluate(model, Xtr, Xte, y_train, y_test, "logistic_regression", mode, args.result_file)

    print("\nLogistic Regression evaluations done.")
