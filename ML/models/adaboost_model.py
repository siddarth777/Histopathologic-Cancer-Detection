...
def main():
    parser = argparse.ArgumentParser(description="Run AdaBoost model")
    ...
    parser.add_argument("--result_file", default="results_adaboost.txt", help="Path to results txt file")
    ...
    for mode in ["all", "selected"]:
        print(f"\nRunning adaboost for mode: {mode}")
        log(f"\n>>> STARTING adaboost __{mode}", args.result_file)

        Xtr = X_train_all if mode == "all" else X_train_sel
        Xte = X_test_all if mode == "all" else X_test_sel

        Xtr_, Xval, ytr_, yval = train_test_split(Xtr, y_train, test_size=0.2, stratify=y_train, random_state=42)
        ...
        evaluate(model, Xtr, Xte, y_train, y_test, "adaboost", mode, args.result_file)

    print("\nAdaBoost evaluations done.")
