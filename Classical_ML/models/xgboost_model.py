...
def main():
    parser = argparse.ArgumentParser(description="Run XGBoost model")
    ...
    parser.add_argument("--result_file", default="results_xgboost.txt", help="Path to results txt file")
    ...
    for mode in ["all", "selected"]:
        print(f"\nRunning xgboost for mode: {mode}")
        log(f"\n>>> STARTING xgboost __{mode}", args.result_file)
        ...
        evaluate(model, Xtr, Xte, y_train, y_test, "xgboost", mode, args.result_file)

    print("\nXGBoost evaluations done.")
